...
export const SIMPLE_QUERY_URL = (querynum: number) =>
  `${BASE_URL}/simple-query/${querynum}`;

//Advanced Query - accepts 1-6
export const ADVANCED_QUERY_URL = (querynum: number) =>
  `${BASE_URL}/advanced-query/${querynum}`;

//View Tables - accepts table names
export const VIEW_TABLES_URL = (querykey: string) =>
  `${BASE_URL}/view-tables/${querykey}`;
